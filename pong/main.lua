...
function love.load()
    ...
    -- variable created to check if the cpu is playing
    cpuPlaying = 0
    -- reaction time of cpu
    reactionTime = 0
...
function love.keypressed(key)
    ...
    -- Change the state of cpu playing
...
function love.draw()
    ...
    if gameState == 'start' then
        ...
        if cpuPlaying == 0 then
            love.graphics.printf('Press C to play against the cpu', 0, 30, VIRTUAL_WIDTH, 'center')
        else
            love.graphics.printf('Press C to play against a second player', 0, 30, VIRTUAL_WIDTH, 'center')
        end
    elseif gameState == 'serve' then
        -- UI messages
        love.graphics.setFont(smallFont)
